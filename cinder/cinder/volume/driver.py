...
volume_opts = [
    ...
    cfg.IntOpt('iser_num_targets',
               default=100,
               help='Number of iser target ids per host'),
...
FLAGS.import_opt('iser_helper', 'cinder.volume.iser')
...
class ISERDriver(VolumeDriver):
    ...
    def initialize_connection(self, volume, connector):
        ...
        if self.configuration.iser_helper == 'lioadm':
    ...
    def _get_iser_initiator(self):
        """Get iser initiator name for this machine"""
    ...
    def copy_image_to_volume(self, context, volume, image_service, image_id):
        ...
        connector = {'initiator': self._get_iser_initiator(),
                     'host': socket.gethostname()}

        iser_properties, volume_path = self._attach_volume(
        ...
        while not os.path.exists(host_device):
            if tries >= self.configuration.num_iser_scan_tries:
                raise exception.CinderException(
                    _("iSER device not found at %s") % (host_device))
